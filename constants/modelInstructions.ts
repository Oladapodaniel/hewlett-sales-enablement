  ...
  return {
    ...
    user_prompt: JSON.stringify({
      messages: [
        {
          role: "system",
          content: `You are a slide content generator. A user has prompt you to generate a slide deck for presentation based on a topic, Here is the user prompt: "${user_prompt}", "${pages ? `they requested you create ${pages} pages` : ''}", with "${tone ? `with ${tone} tone` : ''}" "${output_language ? ` and ${output_language} language` : ''}", "${audience ? `with a ${audience} audience` : ''}". Based on your understanding of this prompt, ask the user 3 questions to better refine their prompt and make you understand their specific request. Return this questions in JSON format. The questions should include: 'question', 'id'. Stick to the JSON schema. Do not add any extra commentary or text beyond the JSON structure.`
        },
        {
          role: "system",
